      ...
      <v-layout wrap>
        ...
        <v-flex xs12 class="text-right">
          <v-tooltip left color="primary">
            ...
            <span>Créer un nouveau type de convention</span>
          </v-tooltip>
        </v-flex>
      </v-layout>
    </v-container>
...
export default {
  name: 'ConventionTypes',
  components: { DeleteDialog },
  data () {
    return {
      title: 'Créer un nouveau type de convention',
      button: 'Créer',
      item: { ID: 0, Name: '' },
      dialog: false,
      delDlg: false,
  ...
  methods: {
    ...
    async delConfirm () {
      await this.$store.dispatch(types.DELETE_CONVENTION_TYPE, this.item.ID)
    },
    modify (item) {
      this.item = { ...item }
    ...
    add (item) {
      this.item.ID = 0
      this.item.Name = ''
      this.dlgTitle = 'Créer un nouveau type de convention'
      this.dlgBtn = 'Créer'
      this.dialog = true
    },
    async confirm (r) {
      const dispatch =
        this.dlgBtn === 'Créer'
          ? types.CREATE_CONVENTION_TYPE
          : types.UPDATE_CONVENTION_TYPE
