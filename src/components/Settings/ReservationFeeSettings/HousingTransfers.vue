<template>
  <v-card>
      ...
      <v-layout wrap>
        ...
        <v-flex xs12 class="text-right">
          <v-tooltip left color="primary">
            ...
            <span>Créer un nouveau type de cession</span>
          </v-tooltip>
        </v-flex>
      </v-layout>
    </v-container>
    ...
    <delete-dialog
      v-model="delDlg"
      sentence="Supprimer le type de cession définitivement ?"
      @confirm="delConfirm"
    />
  </v-card>
</template>
...
export default {
  ...
  data () {
    return {
      title: 'Créer un nouveau type de cession',
      button: 'Créer',
      item: { ID: 0, Name: '' },
      dialog: false,
      delDlg: false,
  ...
  methods: {
    ...
    async delConfirm () {
      await this.$store.dispatch(types.DELETE_HOUSING_TRANSFER, this.item.ID)
    },
    modify (item) {
      this.item = { ...item }
      this.dlgTitle = 'Modifier le type de cession'
    ...
    add (item) {
      this.item.ID = 0
      this.item.Name = ''
      this.dlgTitle = 'Créer un nouveau type de cession'
      this.dlgBtn = 'Créer'
      this.dialog = true
    },
    async confirm (r) {
      const dispatch =
        this.dlgBtn === 'Créer'
          ? types.CREATE_HOUSING_TRANSFER
          : types.UPDATE_HOUSING_TRANSFER
