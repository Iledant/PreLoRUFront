...
export default {
  ...
  computed: {
    ...
    renewProjects () {
      return this.$store.state.renewProjects.renewProjectsList.map(r =>
        ({ ID: r.ID, Name: r.CityName1 + ' - ' + r.Name }))
    },
    disabled () {
      return (
  ...
  methods: {
    confirm () {
      if (!this.disabled) {
        ...
        const rp = this.renewProjects.find(r => r.ID === this.item.KindID)
