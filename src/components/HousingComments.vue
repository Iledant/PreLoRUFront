...
export default {
  ...
  methods: {
    ...
    async delConfirm () {
      await this.$store.dispatch(types.DELETE_HOUSING_COMMENT, this.item.ID)
    },
    modify (item) {
      this.item = { ...item }
      this.dlgTitle = 'Modifier le commentaire'
    ...
    add (item) {
      this.item.ID = 0
      this.item.Name = ''
      this.dlgTitle = 'Créer un nouveau commentaire'
      this.dlgBtn = 'Créer'
      this.dialog = true
    },
    async confirm (r) {
      const dispatch =
        this.dlgBtn === 'Créer'
          ? types.CREATE_HOUSING_COMMENT
          : types.UPDATE_HOUSING_COMMENT
